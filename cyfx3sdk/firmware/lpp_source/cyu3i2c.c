...
#define FX3_I2C_SCL_PIN         (58)
#define FX3_I2C_SDA_PIN         (59)

/* Use the SCL/SDA pins as GPIOs to send an I2C Stop Condition. */
CyU3PReturnStatus_t
CyU3PI2cSendStopCondition (
        void)
{
    CyU3PGpioSimpleConfig_t conf;
    CyU3PReturnStatus_t stat;

    CyU3PDeviceGpioOverride (FX3_I2C_SCL_PIN, CyTrue);
    CyU3PDeviceGpioOverride (FX3_I2C_SDA_PIN, CyTrue);

    /* Keep SCL high, and trigger a low to high transition on SDA. */
    conf.driveHighEn = CyTrue;
    conf.driveLowEn  = CyTrue;
    conf.intrMode    = CY_U3P_GPIO_NO_INTR;
    conf.inputEn     = CyFalse;
    conf.outValue    = CyTrue;

    stat = CyU3PGpioSetSimpleConfig (FX3_I2C_SCL_PIN, &conf);
    if (stat == CY_U3P_SUCCESS)
    {
        conf.outValue = CyFalse;
        stat = CyU3PGpioSetSimpleConfig (FX3_I2C_SDA_PIN, &conf);
        if (stat == CY_U3P_SUCCESS)
        {
            conf.outValue = CyTrue;
            stat = CyU3PGpioSetSimpleConfig (FX3_I2C_SDA_PIN, &conf);
    ...
    CyU3PDeviceGpioRestore (FX3_I2C_SDA_PIN);
    CyU3PDeviceGpioRestore (FX3_I2C_SCL_PIN);
    return stat;
}
...
CyU3PI2cErrorRecovery (CyBool_t isClearFifo)
{
    ...
    /* Make sure we free the bus at this stage. */
    CyU3PI2cSendStopCondition ();
