/*
 ## Cypress FX3S Application Header file (cyfx3s_raid0.h)
 ## ===========================
 ##
...
/* This file contains the constants and definitions for the RAID-0 application example. */

#ifndef _INCLUDED_CYFX3S_RAID0_H_
#define _INCLUDED_CYFX3S_RAID0_H_
...
#define CY_FX_MSC_S0_WRITE_SOCKET       (0)             /* SIB 0 socket is used for writes to S0 card. */
#define CY_FX_MSC_S1_WRITE_SOCKET       (1)             /* SIB 1 socket is used for writes to S1 card. */
#define CY_FX_MSC_S0_READ_SOCKET        (2)             /* SIB 2 socket is used for reads from S0 card. */
#define CY_FX_MSC_S1_READ_SOCKET        (3)             /* SIB 3 socket is used for reads from S1 card. */

#define CY_FX_SIB_SECTOR_SIZE           (512)           /* Actual storage device sector size. */
#define CY_FX_USB_MAXPKT_SIZE           (1024)          /* Actual USB endpoint max packet size. */

#define CY_FX_RAID_BLOCK_SIZE           (4096)          /* Logical block size for RAID disk is 4 KB. */

/* Burst size to be set for the USB endpoint. */
#define CY_FX_RAID_EP_BURST_SIZE        (CY_FX_RAID_BLOCK_SIZE / CY_FX_USB_MAXPKT_SIZE)

/* Multiplying factor to be applied when making SIB read/write requests. */
#define CY_FX_RAID_SECTOR_MULTIPLIER    (CY_FX_RAID_BLOCK_SIZE / CY_FX_SIB_SECTOR_SIZE)

#define CY_FX_RAID_DMA_BUF_COUNT        (4)             /* DMA buffers per socket in each direction. */
...
#define CY_FX_MSC_WR_COMMIT_EVENT_FLAG          (1 << 7)        /* SIB write commit event flag. */
...
#endif /* _INCLUDED_CYFX3S_RAID0_H_ */
