/*
 ## Cypress USB 3.0 Platform source file (cyfxbulklpautoonetomany.c)
 ## ===========================
 ##
...
/* This file illustrates the bulkloop application example using the DMA AUTO ONE TO MANY mode */

/*
   This example illustrates the use of the FX3 firmware APIs to implement a data loopback
   application over three USB bulk endpoints.

   The device enumerates as a vendor specific USB device with three bulk endpoints 
   (1-OUT, 1-IN and 2-IN).  The application loops back any data that it receives on the bulk
   OUT endpoint to the two bulk IN endpoints in an interleaved fashion.

   The loopback is achieved with the help of a multichannel DMA AUTO_ONE_TO_MANY which is created
...
#include "cyu3uart.h"
#include "cyu3utils.h"
#include "cyfxbulklpautoonetomany.h"
        ...
        void)
{
    ...
    /* Create a DMA AUTO_ONE_TO_MANY channel between
    ...
    apiRetStatus = CyU3PDmaMultiChannelCreate (&glChHandleBulkLp,
            CY_U3P_DMA_TYPE_AUTO_ONE_TO_MANY, &dmaCfg);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&BulkLpAppThread,           /* Bulk loop App Thread structure */
                          "21:Bulk_loop_AUTO_ONE_TO_MANY",         /* Thread ID and Thread name */
