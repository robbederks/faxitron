/*
 ## Cypress USB 3.0 Platform source file (cyfxbulklpmulticast.c)
 ## ===========================
 ##
...
/* This file illustrates the bulkloop application example using the DMA MULTICAST mode */

/*
   This example illustrates a loopback mechanism between three USB bulk endpoints. The
   example comprises of vendor class USB enumeration descriptors with three bulk endpoints.
   
   The device enumerates with 3 bulk endpoints (1-OUT, 1-IN and 2-IN). The data is 
   received on EP1 OUT and is send out on both EP1 IN and EP2 IN. Both endpoints receive
   the same data. The loopback is achieved with the help of a multichannel DMA MULTCAST
   which is created connecting the 3 endpoints. This application receives data on the
   1-OUT endpoint from the USB host. CPU is signalled of the incoming data via a DMA callback.
   CPU then sends the same data back to the host on both EP1 IN and EP2 IN.

   The DMA buffer size is defined based on the USB speed. 64 for full speed, 512 for high speed
   and 1024 for super speed. CY_FX_BULKLP_DMA_BUF_COUNT in the header file defines the number
   of DMA buffers.
 */
...
CyBool_t             glChnModified = CyFalse;
        ...
        CyU3PDmaCBInput_t    *input)    /* Callback status.           */
{
    ...
    {
        CyU3PDebugPrint (4, "Transfer completed\r\n");
        if (glChnModified)
        {
            glChnModified = CyFalse;
            CyU3PDmaMultiChannelReset (chHandle);
            CyU3PDmaMulticastSocketSelect (chHandle, 3);
        }
        CyU3PDmaMultiChannelSetXfer (chHandle, CY_FX_BULKLP_DMA_TX_SIZE, 0);
    }
}
        ...
        void)
{
    ...
    /* Consumer 1 endpoint configuration */
    ...
    /* Consumer 2 endpoint configuration */
    ...
    /* Create a DMA MULTICAST channel between
    ...
    apiRetStatus = CyU3PDmaMulticastSocketSelect (&glChHandleBulkLp, 0x03);
    ...
    )
{
    ...
    if (bType == CY_U3P_USB_STANDARD_RQT)
    {
        ...
        {
            if ((wIndex == CY_FX_EP_PRODUCER) || (wIndex == CY_FX_EP_CONSUMER_1)
                    || (wIndex == CY_FX_EP_CONSUMER_2))
            {
                if (glIsApplnActive)
                {
                    CyU3PUsbSetEpNak (CY_FX_EP_PRODUCER, CyTrue);
                    CyU3PUsbSetEpNak (CY_FX_EP_CONSUMER_1, CyTrue);
    ...
    if ((bType == CY_U3P_USB_VENDOR_RQT) && (bRequest == 0x77))
    {
        glChnModified = CyTrue;
        CyU3PUsbAckSetup ();

        CyU3PDebugPrint (4, "Vendor request\r\n");
        CyU3PDmaMulticastDisableConsumer (&glChHandleBulkLp, 1);
        isHandled = CyTrue;
    }
...
CyFxBulkLpApplnInit (void)
{
    ...
    /* Enable the creation of DMA multicast channels. */
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&BulkLpAppThread,           /* Bulk loop App Thread structure */
                          "21:Bulk_loop_MULTICAST",                /* Thread ID and Thread name */
