...
volatile uint8_t  gVendorRqtCnt = 0;
volatile CyBool_t gRstRqt = CyFalse;
    ...
    )
{
    ...
    if (bType == CY_U3P_USB_STANDARD_RQT)
    {
        ...
        {
            if ((wIndex == CY_FX_EP_PRODUCER_1) || (wIndex == CY_FX_EP_PRODUCER_2) || (wIndex == CY_FX_EP_CONSUMER))
            {
                if (glIsApplnActive)
                {
    ...
    if (bType == CY_U3P_USB_VENDOR_RQT)
    {
        if (bRequest == 0x76)
        {
            glEp0Buffer[0] = gVendorRqtCnt;
            glEp0Buffer[1] = 0xDE;
            glEp0Buffer[2] = 0x5A;
            glEp0Buffer[3] = ~gVendorRqtCnt;
