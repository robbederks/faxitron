/*
 ## Cypress USB 3.0 Platform source file (cyfxbulklpmanonetomany.c)
 ## ===========================
 ##
...
/* This file illustrates the bulkloop application example using the DMA MANUAL ONE TO MANY mode */

/*
   This example illustrates a loopback mechanism between three USB bulk endpoints. The example
   comprises of vendor class USB enumeration descriptors with three bulk endpoints.
   
   A bulk OUT endpoint acts as the producer of data from the host. Two bulk IN endpoints act
   as the consumers of data to the host. The loopback is achieved with the help of a DMA
   MANUAL ONE TO MANY channel. DMA channel is created between the producer USB bulk endpoint
   and the consumer USB bulk endpoints. Data is transferred from the host into the producer
   endpoint. This data reception is notified to the CPU. The CPU then commits the data to the
   consumer endpoints and thereby the data gets transferred to the host. The buffer transfers
   from Producer to Consumers happens in an interleaved fashion. In this mode CPU is involved
   in commiting the data recevied from the producer to the consumer.
...
#include "cyu3dma.h"
#include "cyu3error.h"
#include "cyfxbulklpmanonetomany.h"
#include "cyu3usb.h"
#include "cyu3uart.h"
#include "cyu3utils.h"
        ...
        void)
{
    ...
    /* Create a DMA MANUAL_ONE_TO_MANY channel between
    ...
    apiRetStatus = CyU3PDmaMultiChannelCreate (&glChHandleBulkLp,
            CY_U3P_DMA_TYPE_MANUAL_ONE_TO_MANY, &dmaCfg);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    ...
    )
{
    ...
    if (bType == CY_U3P_USB_STANDARD_RQT)
    {
        ...
        {
            if ((wIndex == CY_FX_EP_PRODUCER) || (wIndex == CY_FX_EP_CONSUMER_1) || (wIndex == CY_FX_EP_CONSUMER_2))
            {
                if (glIsApplnActive)
                {
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&BulkLpAppThread,           /* Bulk loop App Thread structure */
                          "21:Bulk_loop_MANUAL_ONE_TO_MANY",       /* Thread ID and Thread name */
