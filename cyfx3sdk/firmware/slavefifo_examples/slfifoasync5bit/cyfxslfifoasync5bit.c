/*
 ## Cypress USB 3.0 Platform source file (cyfxslfifoasync.c)
 ## ===========================
 ##
...
/* This file illustrates the Slave FIFO Asynchronous mode example */
        ...
        void)
{
    ...
    * DMA size is set based on the USB speed. */
    ...
    for(i=0;i<(glNumberOfChannels/2);i++)
    {
        ...
        /* Set DMA channel transfer size. */
        apiRetStatus = CyU3PDmaChannelSetXfer (&glChHandleSlFifoPtoU[i], CY_FX_SLFIFO_DMA_TX_SIZE);
        if (apiRetStatus != CY_U3P_SUCCESS)
        {
            CyU3PDebugPrint (4, "CyU3PDmaChannelSetXfer Failed, Error code = %d\n", apiRetStatus);
            CyFxAppErrorHandler(apiRetStatus);
                ...
                && (wValue == CY_U3P_USBX_FS_EP_HALT))
        {
            if (glIsApplnActive)
            {
                for (i = 0; i < (glNumberOfChannels / 2); i++)
                {
                    ...
                    {
                        CyU3PUsbSetEpNak (wIndex, CyTrue);
                        CyU3PBusyWait (125);
                        ...
                        CyU3PUsbSetEpNak (wIndex, CyFalse);
                        break;
                    }
...
CyFxSlFifoApplnInit (void)
{
    ...
    apiRetStatus = CyU3PGpifLoad (&Async_Slave_Fifo_5Bit_CyFxGpifConfig);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    ...
    apiRetStatus = CyU3PGpifSMStart (ASYNC_SLAVE_FIFO_5BIT_RESET, ASYNC_SLAVE_FIFO_5BIT_ALPHA_RESET);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&slFifoAppThread,           /* Slave FIFO app thread structure */
                          "21:Slave_FIFO_Async_5Bit",                   /* Thread ID and thread name */
