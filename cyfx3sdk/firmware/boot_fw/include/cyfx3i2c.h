...
/** \brief Set the timeout duration for I2C byte mode data transfers.

    <B>Description</B>\n
    The I2C byte mode transfer functions (CyFx3BootI2cSendCommand, CyFx3BootI2cTransmitBytes
    and CyFx3BootI2cReceiveBytes) are implemented using a poll loop which continually checks
    for the completion of the transfer. The delay for each of these loops will be approximately
    2 us, and the default value of the maximum loop count is 4095. This API can be used to
    change the timeout loop count to a different value.
