...
typedef enum CyU3PPartNumber_t
{
    CYPART_USB3014 = 0,                 /* CYUSB3014: 512 KB RAM; GPIF can be 32 bit; UART, SPI and I2S supported.
                                                      OTG and USB host mode supported. */
...
/* Summary
   ...
   the global variables. Also if the scatter load option is used, then the 
   firmware should be capable of skipping this step as the image is already
   loaded.
...
/* Summary
   ...
   and the RTOS has started. No peripheral module is expected to function 
   in this state. All modules are expected to be stopped and all DMA channels
   destroyed before entering this state. In this mode, the device is held in
   reset and only very minimal part of the device is left powered. This function
   does not return until the device has already resumed normal operation.

   Return value
   CY_U3P_SUCCESS - if the call is successful
   CY_U3P_ERROR_INVALID_CALLER - if called from interrupt context
   CY_U3P_ERROR_MEMORY_ERROR - memory allocation failure
