...
/** \brief Wraps up the current active buffer for the channel from the producer side.
    ...
    **Note**\n
    This API will not work on PIB sockets if the data count in the buffer is less than
    48 bytes. In such a case, the GPIF state machine should be modified to ensure that
    the buffer is automatically committed (using the COMMIT) action.
...
/** \brief Wraps up the current active buffer on the producer socket.
    ...
    **Note**\n
    This API will not work on PIB sockets if the data size is less than 48 bytes.
    Use the GPIF state-machine to trigger the buffer wrap-up in this case.
