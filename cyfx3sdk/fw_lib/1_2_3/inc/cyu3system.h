...
/** \file cyu3system.h
    ...
    models to system designers. The FX3 device is the latest addition to this
    family, and provide the high performance USB 3.0 SuperSpeed capability
    ...
    **Description**\n
    ...
    transfers at speeds greater than 400 MBps. An integrated USB 2.0 OTG controller
    enables applications that need dual role usage scenarios. There is up to 512 KB
    of on-chip SRAM for code and data usage.
    ...
    customers to leverage the features of the FX3 device, and build a full
    featured design in a short time.

    **Description**\n
    The FX3 SDK provides a firmware framework that allows FX3 customers to
    ...
    application examples that are put together using these API. Please refer to the
    FX3 Programmer's Manual for details on the various application examples.

    The rest of this manual provides detailed information about the API provided by the
    FX3 SDK and the data structures used by them.
...
/** \brief IO matrix configuration parameters.
    ...
    **Description**\n
    The EZ-USB FX3 device has a flexible IO architecture that allows each IO Pin to serve
    ...
    This structure captures the desired IO configuration for the FX3 device as a whole.
...
/** \brief Configures the IO matrix for the device.
    ...
    **Description**\n
    The FX3 devices have upto 61 IO pins that can be configured to function in a
    ...
    Some devices in the family, such as the CYUSB3011 and CYUSB3013 devices; support only
    a 16 bit wide GPIF interface. The isDQ32Bit field should be set to false when using
    these devices.
