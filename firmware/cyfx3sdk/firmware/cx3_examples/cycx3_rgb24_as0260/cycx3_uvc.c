...
/* This application example implements a RGB888 video camera over the UVC 1.1 protocol on the CX3 using
 * an Aptina AS0260 image sensor (streaming RGB 565 stream) over Mipi CSI2.
 *
 * The example provides uncompressed 16Bit RGB565 1920x1080 @30 fps
...
static volatile int32_t glDMATxCount = 0;      /* Counter used to count the Dma Transfers */
...
static volatile uint32_t glFailFrameCount = 0;                  /* Number of video frames that have failed. Also to support Mac OS*/
static CyU3PTimer        UvcTimer;                      /* Timer used to track frame transfer time. */

static void
CyCx3AppProgressTimer (
        uint32_t arg)
{
    /* This frame has taken too long to complete. Notify the thread to abort and restart streaming. */
        ...
        )
{
    ...
    switch (CyU3PUsbGetSpeed ())
    {
        case CY_U3P_SUPER_SPEED:
            {
                switch (resolutionIndex)
                {
                    ...
                    case 0x03: /* VGA 60 fps */
                        {
#ifdef CX3_DEBUG_ENABLED
                            CyU3PDebugPrint (4, "\n\rUSBStpCB:Write SS-VGA Settings.");
#endif
                            ...
                            if (status != CY_U3P_SUCCESS)
                            {
                                CyU3PDebugPrint (4, "\n\rUSBStpCB:SetIntfParams SS3 Err = 0x%x", status);
                            }
                        }
                        break;
        ...
        default:
            {
                /* Only QVGA is supported in Hi-Speed and Full Speed. */
#ifdef CX3_DEBUG_ENABLED
                CyU3PDebugPrint (4, "\n\rUSBStpCB:Write QVGA Settings.");
#endif
                /* Write QVGA Settings */
        ...
        uint16_t wValue)
{
    ...
    if (status != CY_U3P_SUCCESS)
    {
        /* No further error handling as we cannot stall EP0 after calling CyU3PUsbGetEP0Data */
    ...
    {
#ifdef CX3_DEBUG_ENABLED
        CyU3PDebugPrint (4, "\n\rUSBStpCB:Probe_Ctrl set success.");
        CyU3PDebugPrint (4, "\n\rFmt = %d, Frm = %d.", glCommitCtrl[2], glCommitCtrl[3]);
#endif
    ...
    else
    {
        ...
        {
#ifdef CX3_DEBUG_ENABLED
            CyU3PDebugPrint (4, "\n\rUSBStpCB:Commit_Ctrl success");
            CyU3PDebugPrint (4, "\n\rFmt = %d, Frm = %d ", glCommitCtrl[2], glCommitCtrl[3]);
#endif

            CyCx3AppImageSensorSetVideoResolution (glCommitCtrl[3]);
            
            CyCx3_ImageSensor_Set_Format (SENSOR_RGB565);
            
            /* If we were already streaming, stop and restart streaming. */
            if (glIsApplnActive)
            {
#ifdef CX3_DEBUG_ENABLED
        ...
        )
{
    ...
    if (bType == CY_U3P_USB_CLASS_RQT)
    {
        ...
        if ((wIndex & 0x00FF) == CX3_APP_STREAM_INTERFACE)
        {
            switch (bRequest)
            {
                ...
                case CX3_USB_APP_GET_LEN_REQ:
                    ...
                    /* As we only have a single setting for each video format, we send the same data in response
                     * to GET_CUR, GET_DEF, * GET_MIN and GET_MAX requests.
                     */
        ...
        if ((wIndex & 0x00FF) == CX3_APP_CONTROL_INTERFACE)
        {
            ...
            if (wValue == CX3_APP_VC_REQUEST_ERROR_CODE_CONTROL)
            {
        ...
        void)
{
    ...
    /* Configure the Fixed Function GPIF on the CX3 to use a 24 bit bus, and
     * a DMA Buffer of size CX3_APP_DATA_BUF_SIZE
     */
    status = CyU3PMipicsiGpifLoad (CY_U3P_MIPICSI_BUS_24, CX3_APP_DATA_BUF_SIZE);
    if (status != CY_U3P_SUCCESS)
    {
...
#ifdef RESET_TIMER_ENABLE
    CyU3PTimerCreate (&UvcTimer, CyCx3AppProgressTimer, 0x00, TIMER_PERIOD, 0, CYU3P_NO_ACTIVATE);
#endif
        ...
        uint32_t input)
{
    ...
    for (;;)
    {
        ...
        if (status == CY_U3P_SUCCESS)
        {
            ...
            {
            	CyU3PDebugPrint(4,"DMA_RESET\r\n");
                /* Frame timed out. Abort and restart streaming. */
                if (glIsApplnActive)
                {
                    CyCx3AppStop ();
                }
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&cx3AppThread,   /* UVC Thread structure */
    ...
    retThrdCreate = CyU3PThreadCreate (&cx3MipiErrorThread,     /* UVC Thread structure */
    ...
    retThrdCreate = CyU3PEventCreate (&glMipiErrorEvent);
    if (retThrdCreate != 0)
        goto StartupError;
#endif
...
StartupError:
    {
        /* Creating threads and other services for the application failed. As this is a fatal error,
         * we cannot continue with the application.
         */
