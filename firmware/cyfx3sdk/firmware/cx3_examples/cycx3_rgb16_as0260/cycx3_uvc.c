...
/* This application example implements a RGB565 video camera over the UVC 1.1 protocol on the CX3 using an
 * Aptina AS0260 image sensor. The example provides uncompressed 16Bit RGB565 1920x1080 @30 fps
...
static CyU3PTimer Cx3ResetTimer;                        /* Timer used to track completion of video frames. */

static void
CyCx3AppProgressTimer (
        uint32_t arg)
{
    /* This frame has taken too long to complete. Notify the thread to abort the frame. */
        ...
        uint32_t input)
{
    ...
    uint16_t wakeReason;
    uint32_t eventFlag;
    uint32_t eventMask = CX3_USB_SUSP_EVENT_FLAG | CX3_DMA_RESET_EVENT;
    ...
    for (;;)
    {
        status = CyU3PEventGet (&glCx3Event, eventMask, CYU3P_EVENT_OR_CLEAR, &eventFlag, CYU3P_WAIT_FOREVER);
        if (status == CY_U3P_SUCCESS)
        {
            if (eventFlag & CX3_DMA_RESET_EVENT)
            {
                /* Frame transfer has timed out. Abort and re-start streaming. */
                if (glIsApplnActive)
                {
                    CyCx3AppStop ();
                }
