/*
 ## Cypress USB 3.0 Platform source file (cyfxbulklpmanualadd.c)
 ## ===========================
 ##
...
/* This file illustrates the bulkloop application example using the DMA MANUAL mode with header / footer addition. */
...
/*
   ...
   The CPU then adds the header and footer and then commits the DMA buffer received so that the data is
   transferred to the consumer endpoint and thereby to the host. 

   The DMA buffer size is defined based on the USB speed. (64 + 8) for full speed, (512 + 8) for high speed
   and (1024 + 8) for super speed. CY_FX_BULKLP_DMA_BUF_COUNT in the header file defines the number of DMA
   buffers.
 */
        ...
        CyU3PDmaCBInput_t *input)    /* Callback status.           */
{
    ...
    if (type == CY_U3P_DMA_CB_PROD_EVENT)
    {
         ...
         * Update the buffer with header and footer. */
        ptr = input->buffer_p.buffer - CY_FX_DMA_HEADER_SIZE;
        CyU3PMemSet (ptr, (uint8_t)CY_FX_HEADER_DATA, CY_FX_DMA_HEADER_SIZE);
        /* Update count to include header and data. */
        count = input->buffer_p.count + CY_FX_DMA_HEADER_SIZE; 
        ptr += count;
        CyU3PMemSet (ptr, (uint8_t)CY_FX_FOOTER_DATA, CY_FX_DMA_FOOTER_SIZE);
        /* Update the count to include the footer. */
        count += CY_FX_DMA_FOOTER_SIZE;
        ...
        void)
{
    ...
    dmaCfg.size  = (size + CY_FX_DMA_HEADER_SIZE + CY_FX_DMA_FOOTER_SIZE);
    ...
    /* Allow the producer socket to see the buffer with header offset. */
    dmaCfg.prodHeader = CY_FX_DMA_HEADER_SIZE;
    /* Overallocate footer for producer socket. */
    dmaCfg.prodFooter = CY_FX_DMA_FOOTER_SIZE;
    dmaCfg.consHeader = 0;
