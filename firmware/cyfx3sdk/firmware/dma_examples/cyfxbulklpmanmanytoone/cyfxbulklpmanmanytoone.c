/*
 ## Cypress USB 3.0 Platform source file (cyfxbulklpmanmanytoone.c)
 ## ===========================
 ##
...
/* This file illustrates the bulkloop application example using the DMA MANUAL_MANY_TO_ONE mode. */
...
/*
   ...
   Two bulk OUT endpoints act as the producers of data from the host. A bulk IN endpint acts as
   the consumer of data to the host. The loopback is achieved with the help of a DMA MANUAL_MANY_TO_ONE
   channel. DMA channel is created between the producer USB bulk endpoints and the consumer USB bulk endpoint.

   Data is transferred from the host into the producer endpoints. CPU gets notified of the incoming buffer.
   CPU then commits the buffer to the consumer endpoint and to the host. Data transfer from the producer
   endpoints to the consumer endpoint happen in an interleaved fashion.
...
#include "cyu3dma.h"
#include "cyu3error.h"
#include "cyfxbulklpmanmanytoone.h"
#include "cyu3usb.h"
#include "cyu3uart.h"
#include "cyu3utils.h"
    ...
    if (bType == CY_U3P_USB_STANDARD_RQT)
    {
                ...
                && (wValue == CY_U3P_USBX_FS_EP_HALT))
        {
            if ((wIndex == CY_FX_EP_PRODUCER_1) || (wIndex == CY_FX_EP_PRODUCER_2)
                    || (wIndex == CY_FX_EP_CONSUMER))
            {
                if (glIsApplnActive)
                {
                    CyU3PUsbSetEpNak (CY_FX_EP_PRODUCER_1, CyTrue);
                    ...
                    CyU3PUsbSetEpNak (CY_FX_EP_PRODUCER_1, CyFalse);
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&BulkLpAppThread,           /* Bulk loop App Thread structure */
                          "21:Bulk_loop_MANUAL_MANY_TO_ONE",       /* Thread ID and Thread name */
