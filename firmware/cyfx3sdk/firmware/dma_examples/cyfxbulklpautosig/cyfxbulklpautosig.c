/*
 ## Cypress USB 3.0 Platform source file (cyfxbulklpautosig.c)
 ## ===========================
 ##
...
/* This file illustrates the bulkloop application example using the DMA AUTO mode with signalling */
...
/*
   ...
   of data to the host. The loopback is achieved with the help of a DMA AUTO_SIGNAL channel. DMA AUTO_SIGNAL
   channel is created between the producer USB bulk endpoint and the consumer USB bulk endpoint. Data is 
   transferred from the host into the producer endpoint which is then directly transferred to the consumer
   endpoint by the DMA engine.

   Upon every reception of data in the DMA buffer from the host the CPU is signalled using DMA callbacks.
   The CPU could choose to perform any operations in the callback. However the data transfer to the consumer
   endpoint is not affected. In this example a DMA transaction counter is implemented in the callback to
   track each data transfer. CPU is not involved in the data transfer.
...
uint32_t glDmaTxCount = 0;               /* Counter to track the number of buffers sent out. */
        ...
        void)
{
    ...
    /* Create a DMA AUTO_SIGNAL Channel between two sockets of the U port.
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&BulkLpAppThread,           /* Bulk loop App Thread structure */
                          "21:Bulk_loop_AUTO_SIGNAL",              /* Thread ID and Thread name */
