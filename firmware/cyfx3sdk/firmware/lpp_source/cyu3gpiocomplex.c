        ...
        uint32_t period)
{
    ...
    /* Disable the GPIO first and change the OUT value to 0. */
    GPIO->lpp_gpio_pin[index].status &= ~CY_U3P_LPP_GPIO_ENABLE;
    GPIO->lpp_gpio_pin[index].status &= ~CY_U3P_LPP_GPIO_OUT_VALUE;

    /* Update the period and threshold values. */
    GPIO->lpp_gpio_pin[index].threshold = threshold;
    GPIO->lpp_gpio_pin[index].period    = period;

    /* Now enable the GPIO again. */
    GPIO->lpp_gpio_pin[index].status |= CY_U3P_LPP_GPIO_ENABLE;
