...
static CyU3PTimer  glRaidAppTimer;                              /* MSC application timer object. */
...
static volatile uint32_t glRaidIdleTime  = 0;                   /* Time in seconds since last write operation. */
...
SkipS1Write:
                ...
                {
                    /* Clear the write delay timer as new write has been started. */
                    glRaidIdleTime = 0;
                }
            }
            break;
        ...
        void)
{
    ...
    if (unitCapacity != 0)
    {
        ...
        if ((status == CY_U3P_SUCCESS) && (unitInfo.location == CY_U3P_SIB_LOCATION_USER))
        {
            ...
            glLunNumBlks = ((unitCapacity / CY_FX_RAID_SECTOR_MULTIPLIER) * 2) - 1;
        }
    }
...
/* Callback that checks for SD card idle status and initiates write commit operation. */
static void
CyFxRaidAppTimerCallback(
        uint32_t param)
{
    glRaidIdleTime++;
    if (glRaidIdleTime > CY_FX_MSC_APP_MAX_COMMIT_DELAY)
    {
        CyU3PEventSet (&glRaidAppEvent, CY_FX_MSC_WR_COMMIT_EVENT_FLAG, CYU3P_EVENT_OR);
    }
}
        ...
        void)
{
    ...
    txApiRetStatus = CyU3PTimerCreate (&glRaidAppTimer, CyFxRaidAppTimerCallback, 0, 1000, 1000, CYU3P_AUTO_ACTIVATE);
    if (txApiRetStatus != 0)
    {
        /* Error Handling */
        CyFxAppErrorHandler (txApiRetStatus);
        ...
        uint32_t input)
{
    ...
    uint32_t evMask = CY_FX_MSC_SETCONF_EVENT_FLAG | CY_FX_MSC_RESET_EVENT_FLAG | CY_FX_MSC_CBW_EVENT_FLAG |
        ...
        CY_FX_MSC_WR_COMMIT_EVENT_FLAG;
    uint32_t evStat;
    uint32_t cardStat;
    uint16_t wakeReason;
    ...
    for (;;)
    {
        ...
        if (status == CY_U3P_SUCCESS)
        {
            ...
            {
                /* We can only commit write operations when the MSC state machine is idle. */
                if ((glMscState != CY_FX_MSC_STATE_DATA) && (glMscState != CY_FX_MSC_STATE_STATUS))
                {
                    CyU3PSibCommitReadWrite (0);
                    CyU3PSibCommitReadWrite (1);
                    glRaidIdleTime = 0;
                }
                else
                {
                    /* Queue the commit task again. */
                    CyU3PEventSet (&glRaidAppEvent, CY_FX_MSC_WR_COMMIT_EVENT_FLAG, CYU3P_EVENT_OR);
                }
            }
