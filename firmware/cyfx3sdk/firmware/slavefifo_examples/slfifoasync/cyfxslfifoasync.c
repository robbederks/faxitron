...
CyFxSlFifoApplnInit (void)
{
    ...
    apiRetStatus = CyU3PGpifLoad (&Async_Slave_Fifo_2Bit_CyFxGpifConfig);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
    ...
    apiRetStatus = CyU3PGpifSMStart (ASYNC_SLAVE_FIFO_2BIT_RESET, ASYNC_SLAVE_FIFO_2BIT_ALPHA_RESET);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        ...
        void)
{
    ...
    retThrdCreate = CyU3PThreadCreate (&slFifoAppThread,           /* Slave FIFO app thread structure */
                          "21:Slave_FIFO_async",                   /* Thread ID and thread name */
