        ...
        void)
{
    ...
    for (index = 0; index < CY_FX_ISOSRCSINK_DMA_BUF_COUNT; index++)
    {
        ...
        for (j = 0; j < CY_FX_ISO_PKTS * CY_FX_ISO_BURST * CY_FX_DMA_MULTIPLIER; j++)
            CyU3PMemSet (buf_p.buffer + j * CY_FX_ISO_MAXPKT, CY_FX_ISOSRCSINK_PATTERN + j, CY_FX_ISO_MAXPKT);

        buf_p.buffer[0] = glDmaPktsCommitted;
        ...
        uint32_t input)
{
    ...
    for (;;)
    {
        ...
        if (status == CY_U3P_SUCCESS)
        {
            dmaInfo.buffer[0] = glDmaPktsCommitted;
