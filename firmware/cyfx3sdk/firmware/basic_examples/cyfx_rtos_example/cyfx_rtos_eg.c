        ...
        uint32_t input)
{
    ...
    if (status != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (2, "Error: Failed to register thread monitoring GPIO, status = %d\r\n", status);
    }
    ...
    if (status != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (2, "Error: Failed to register mutex monitoring GPIO, status = %d\r\n", status);
    }
    ...
    if (status != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (2, "Error: Failed to register semaphore monitoring GPIO, status = %d\r\n", status);
    }
    ...
    if (status != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (2, "Error: Failed to register event group monitoring GPIO, status = %d\r\n", status);
    }
    ...
    for (;;)
    {
        ...
        count++;
        if ((count & 0x0F) == 0)
            CyU3PDebugPrint (2, "Thread1: count=%d\r\n", count++);
        if (count == 0)
        ...
        uint32_t input)
{
    ...
    for (;;)
    {
        ...
        CyU3PDebugPrint (2, "Thread2: CPU load=%d, thread1=%d, thread2=%d, drivers=%d\r\n",
        ...
        uint32_t input)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            CyU3PDebugPrint (2, "Thread4: flag=%d\r\n", flag);
            isSet = CyTrue;
        }
    }
}
