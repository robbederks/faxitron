...
static volatile CyBool_t glIsApplnActive = CyFalse;     /* Whether the UAC application is active or not. */
static volatile CyBool_t glIsDevConfigured = CyFalse;   /* Whether the USB device configuration is complete. */
    ...
    )
{
    ...
    switch (evtype)
    {
        case CY_U3P_USB_EVENT_SETCONF:
			...
			CyU3PUsbLPMDisable();
            /* Stop application on SET_CONFIG. It will be started on SET_INTF. */
            if (glIsApplnActive)
                CyFxUacApplnStop ();
            if (evdata != 0)
