        ...
        uint8_t *buffer)
{
    ...
    /* Need to modify the address length and values depending on the SPI device being used. */
    addr[0] = 0x03;
        ...
        uint8_t *data)
{
    ...
    /* Need to modify the address length and values depending on the SPI device used. */
    addr[0] = 0x02;
    addr[1] = (address >> 16) & 0xFF;
    addr[2] = (address >> 8) & 0xFF;
    addr[3] = (address) & 0xFF;
...
bootFromSpi()
{
    ...
    while (isTrue)
    {
        ...
        sectionAddress = (CY_U3P_MAKEDWORD(buffer[7], buffer[6], buffer[5], buffer[4]));
    ...
    if (CY_U3P_MAKEDWORD(buffer[3], buffer[2], buffer[1], buffer[0]) == 0)
    {
        ...
        sectionAddress = (CY_U3P_MAKEDWORD(buffer[7], buffer[6], buffer[5], buffer[4]));
        CyFx3BootJumpToProgramEntry (sectionAddress);
    }
