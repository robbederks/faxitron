...
/** \brief Set the timeout duration for SPI byte mode data transfers.

    <B>Description</B>\n
    The SPI byte mode transfer functions (CyFx3BootSpiTransmitWords and CyFx3BootSpiReceiveWords)
    are implemented using a poll loop which continually checks for the completion of the transfer.
    The delay for each of these loops will be approximately 2 us, and the default value of the
    maximum loop count is 1048575. This API can be used to change the timeout loop count to a
    different value.
